...
def main(argv):
  ...
  if _LOAD_CHECKPOINT_PATH.value is not None:
    ...
    if ckpt_path.is_dir():
      # --- PATCH: if user provided --load_checkpoint_path, don't auto-scan checkpoints ---
      if _LOAD_CHECKPOINT_PATH.value:
        latest_ckpts = []
      else:
              latest_ckpts = list(ckpt_path.glob("*"))
      latest_ckpts = [ckpt for ckpt in latest_ckpts if ckpt.is_dir()]
      latest_ckpts = [p for p in latest_ckpts if str(p.name).isdigit()]
      latest_ckpts.sort(key=lambda x: int(x.name))
      if not latest_ckpts:
        # No numeric checkpoints found (e.g., ocdbt artifacts). Skip auto-latest selection.
        latest_ckpt = None
