...
def iter_render_array(
  ...
  """Yields rendered images frame-by-frame without buffering the full video."""
  renderer = mujoco.Renderer(mj_model, height=height, width=width)
  camera = camera if camera is not None else -1

  if hfield_data is not None:
    mj_model.hfield_data = np.asarray(
        hfield_data.reshape(mj_model.hfield_data.shape)
    )
  ...
  def get_image(state, modify_scn_fn=None) -> np.ndarray:
    ...
    d.ctrl = np.asarray(state.data.ctrl)
    d.time = float(np.asarray(state.data.time))
    d.mocap_pos = np.asarray(state.data.mocap_pos)
    d.mocap_quat = np.asarray(state.data.mocap_quat)
    d.xfrc_applied = np.asarray(state.data.xfrc_applied)
  ...
  try:
    for i, state in enumerate(tqdm.tqdm(trajectory)):
      ...
      yield get_image(state, modify_scene_fn)
  finally:
    renderer.close()
